// generated from projects/poca-card/src/lib/v2/shaders/lygia/color/layer/differenceSourceOver.glsl
...
use: <vec4> layerDifferenceSourceOver(<vec4> src, <vec4> dst)
license:
...
vec4 layerDifferenceSourceOver(vec4 src, vec4 dest) {
    vec4 result = vec4(0.0, 0.0, 0.0, 0.0);

    // Compute difference for RGB channels
    vec3 blendedColor = blendDifference(src.rgb, dest.rgb);
