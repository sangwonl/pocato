// generated from projects/poca-card/src/lib/v2/shaders/lygia/color/layer/colorSourceOver.glsl
...
use: <vec4> layerColorSourceOver(<vec4> src, <vec4> dst)
license:
...
vec4 layerColorSourceOver(vec4 src, vec4 dest) {
    vec4 result = vec4(0.0, 0.0, 0.0, 0.0);

    // Compute color for RGB channels
    vec3 blendedColor = blendColor(src.rgb, dest.rgb);
