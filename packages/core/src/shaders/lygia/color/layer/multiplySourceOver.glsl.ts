// generated from projects/poca-card/src/lib/v2/shaders/lygia/color/layer/multiplySourceOver.glsl
...
use: <vec4> layerMultiplySourceOver(<vec4> src, <vec4> dst)
license:
...
vec4 layerMultiplySourceOver(vec4 src, vec4 dest) {
    vec4 result = vec4(0.0, 0.0, 0.0, 0.0);

    // Compute multiply for RGB channels
    vec3 blendedColor = blendMultiply(src.rgb, dest.rgb);
