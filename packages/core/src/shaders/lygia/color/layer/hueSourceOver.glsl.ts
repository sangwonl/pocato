// generated from projects/poca-card/src/lib/v2/shaders/lygia/color/layer/hueSourceOver.glsl
...
use: <vec4> layerHueSourceOver(<vec4> src, <vec4> dst)
license:
...
vec4 layerHueSourceOver(vec4 src, vec4 dest) {
    vec4 result = vec4(0.0, 0.0, 0.0, 0.0);

    // Compute overlay for RGB channels
    vec3 blendedColor = blendHue(src.rgb, dest.rgb);
